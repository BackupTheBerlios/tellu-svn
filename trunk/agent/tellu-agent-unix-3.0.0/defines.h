...
#define PACK_PKG_1 PACK_ENVS " pkg_info -a -I 2>/dev/null | cut -f 1 -d ' ' | while read a; do b=$(pkg_info -d $a 2>/dev/null | grep '^WWW:' | sed -e 's/^WWW://' -e 's/^.* //'); c=$(pkg_info -s $a 2>/dev/null | grep 'blocks)$' | sed -e 's/[^0-9].*$//'); echo -e \"$a\t\t$c\t$b\"; done"
#define PACK_PKG_2 PACK_ENVS " /usr/sbin/pkg_info -a -I 2>/dev/null | cut -f 1 -d ' ' | while read a; do b=$(/usr/sbin/pkg_info -d $a 2>/dev/null | grep '^WWW:' | sed -e 's/^WWW://' -e 's/^.* //'); c=$(/usr/sbin/pkg_info -s $a 2>/dev/null | grep 'blocks)$' | sed -e 's/[^0-9].*$//'); echo -e \"$a\t\t$c\t$b\"; done"
