...
if($q->param('device') && $q->param('device') ne "") {
	$n = $q->param('device');
}
	...
	if($q->param('note') && $q->param('note') ne "") {
		if($q->param('verb') && $q->param('verb') eq "go") {
			if($q->param('action') && $q->param('action') eq "modify") {
				@r = &sendCommand({ command => "noteModifyDevice", item => $q->param('note'), domain => "", param => $q->param('new_note'), option => "" });
			}
			elsif($q->param('action') && $q->param('action') eq "delete") {
				@r = &sendCommand({ command => "noteDeleteDevice", item => $q->param('note'), domain => "", param => "", option => "" });
			}

			&htmlPage({ template => "popup", title => $WINDOW_TITLE . " - " . $t, script => &deviceNoteFuncs(), header => "", subheader => "", content => $PAGE, slices => $MENU });

			exit(0);
		}

		@r = &sendCommand({ command => "notePullDeviceByID", item => $n, domain => "", param => $q->param('note'), option => "" });
