...
sub peripheralListingDisposedFuncs {
	...
	$r .= "function popupPeripheral(host, slice, width, height) {" . $CONFIG_LINEFEED;
	$r .= " window.open('peripheral_d_popup.pl?dev=' +host+ '&slice=' +slice+ '', '', 'scrollbars,resizable,height=' +height+ ',width=' +width+ '');" . $CONFIG_LINEFEED;
	$r .= "}" . $CONFIG_LINEFEED;
