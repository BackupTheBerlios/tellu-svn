...
int configRead(char *configFile) {
	int i, k;
...
void *configFetch(char *confKeyword, int *i) {
	for(*i = 0; ; *i = *i + 1) {
		if(mainConfigs[*i].confKeyword == NULL) {
			break;
		}
...
int configChangeRoot(char *newRoot) {
	if(chdir(newRoot) != 0) {
		return(-1);
	}
...
int configDaemonize(void) {
	...
	switch(newPid = fork()) {
		...
		case 0:
			if((newSid = setsid()) == -1) {
				warningMessage(ERROR_SLIGHT, "Error occurred while trying to run process in new session");
			}
