...
char *fetchPassword(int getThis, int getType, struct threadInfo * ti) {
	...
	ti->commandInfo.s =
		(ti->handlerArrays[HANDLER_ARRAY_UID].size * 12) +
		(ti->handlerArrays[HANDLER_ARRAY_ITEM].size * 12) +
		(ti->handlerArrays[HANDLER_ARRAY_DOMAIN].size * 12) +
		(ti->handlerArrays[HANDLER_ARRAY_PARAM].size * 12) +
		(ti->handlerArrays[HANDLER_ARRAY_OPTION].size * 12) +
		CONFIG_SPACE_SIZE;
		...
		switch(getThis) {
			...
			case 5:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_PWD_LIST " FROM " TABLE_PWD " WHERE (" TABLECOL_PWD_OWNER " = '%s' OR " TABLECOL_PWD_PUBLIC " = '1') AND " TABLECOL_PWD_OWNER " LIKE '%%%s%%' OR " TABLECOL_PWD_NAME " LIKE '%%%s%%' OR " TABLECOL_PWD_SERVICE " LIKE '%%%s%%' OR " TABLECOL_PWD_USERNAME " LIKE '%%%s%%' OR " TABLECOL_PWD_DESCR " LIKE '%%%s%%' OR " TABLECOL_PWD_NOTE " LIKE '%%%s%%' ORDER BY " TABLECOL_PWD_NAME "%c",
					ti->commandInfo.esc1Buffer,
					ti->commandInfo.esc4Buffer,
					ti->commandInfo.esc4Buffer,
					ti->commandInfo.esc4Buffer,
