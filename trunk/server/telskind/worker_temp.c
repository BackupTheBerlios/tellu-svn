...
char *pullTemp(struct threadInfo * ti) {
	...
	// Get requested temp entry
	return(fetchTemp(1, QUERY_TYPE_PULL, ti));
}

char *pushTemp(struct threadInfo * ti) {
	if(ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer == NULL ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer[0] == 0 ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].size == 0) {
		replyPrepare(ERROR_SLIGHT, ERROR_CLASS_GENERAL, ERROR_CODE_GENERAL_PARAMETERNEEDED, ERROR_MESS_GENERAL_PARAMETERNEEDED, ti);
	...
	// Save new temp entry
	return(fetchTemp(2, QUERY_TYPE_ROUND, ti));
}

char *fetchTemp(int getThis, int getType, struct threadInfo * ti) {
		...
		switch(getThis) {
			case 1:
				...
				snprintf(
					...
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLECOL_TEMP_TEMP " FROM " TABLE_TEMP " WHERE " TABLECOL_TEMP_ID " = '%s'%c",
			...
			case 2:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"INSERT INTO " TABLE_TEMP " (" TABLEKEY_TEMP_TEMP ") VALUES('%s', '%s')%c",
		...
		switch(getType) {
			...
			case QUERY_TYPE_PUSH:
				...
				snprintf(
					ti->logSpace,
					sizeof(ti->logSpace),
					"String \"%s\" modified by \"%s\" using command \"%s\"%c",
			...
			case QUERY_TYPE_ROUND:
				if(dbRound(&ti->storageInfo, ti->commandInfo.statBuffer) == NULL) {
					...
					snprintf(
						ti->logSpace,
						sizeof(ti->logSpace),
						"String created by \"%s\" using command \"%s\", time to live " TIMER_RESOLUTION_TEMP_FLUSH " seconds%c",
						ti->handlerArrays[HANDLER_ARRAY_UID].buffer,
						ti->handlerArrays[HANDLER_ARRAY_COMMAND].buffer,
						0
					);
