...
char *pullMisc(struct threadInfo * ti) {
	...
	// Get requested item
	return(fetchMisc(1, QUERY_TYPE_PULL, ti));
}

char *fetchMisc(int getThis, int getType, struct threadInfo * ti) {
		...
		switch(getThis) {
			case 1:
				...
				snprintf(
					...
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_MISC_MISC " FROM " TABLE_MISC " WHERE " TABLECOL_MISC_CLASS " = '%s' ORDER BY " TABLEORD_MISC_PLACE "%c",
			...
			case QUERY_TYPE_PUSH:
				...
				if(ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer == NULL || ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer[0] == 0 || ti->handlerArrays[HANDLER_ARRAY_PARAM].size == 0) {
					snprintf(
						ti->logSpace,
						sizeof(ti->logSpace),
						"Variable \"%s\" modified by \"%s\" using command \"%s\"%c",
					...
					snprintf(
						ti->logSpace,
						sizeof(ti->logSpace),
						"Variable \"%s\" modified by \"%s\" using command \"%s\" with param \"%s\"%c",
						ti->handlerArrays[HANDLER_ARRAY_ITEM].buffer,
						ti->handlerArrays[HANDLER_ARRAY_UID].buffer,
						ti->handlerArrays[HANDLER_ARRAY_COMMAND].buffer,
