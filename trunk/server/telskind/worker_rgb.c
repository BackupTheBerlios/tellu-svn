...
char *listRGB(struct threadInfo * ti) {
	// List all colors
	return(fetchRGB(1, QUERY_TYPE_PULL, ti));
}

char *pullRGB(struct threadInfo * ti) {
	...
	// Get requested color
	return(fetchRGB(2, QUERY_TYPE_PULL, ti));
}

char *searchRGB(struct threadInfo * ti) {
	if(ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer == NULL ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer[0] == 0 ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].size == 0 ||
	...
	// Search from colors
	return(fetchRGB(3, QUERY_TYPE_PULL, ti));
}

char *fetchRGB(int getThis, int getType, struct threadInfo * ti) {
		...
		switch(getThis) {
			case 1:
				...
				snprintf(
					...
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_RGB_RGB " FROM " TABLE_RGB " ORDER BY " TABLEORD_RGB_NAME "%c",
					0
				);
			...
			case 2:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_RGB_RGB " FROM " TABLE_RGB " WHERE " TABLECOL_RGB_NAME " = '%s'%c",
			...
			case 3:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_RGB_RGB " FROM " TABLE_RGB " WHERE %s LIKE '%%%s%%' ORDER BY " TABLEORD_RGB_NAME "%c",
					ti->commandInfo.esc4Buffer,
			...
			case QUERY_TYPE_PUSH:
				...
				if(ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer == NULL || ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer[0] == 0 || ti->handlerArrays[HANDLER_ARRAY_PARAM].size == 0) {
					snprintf(
						ti->logSpace,
						sizeof(ti->logSpace),
						"Color \"%s\" modified by \"%s\" using command \"%s\"%c",
					...
					snprintf(
						ti->logSpace,
						sizeof(ti->logSpace),
						"Color \"%s\" modified by \"%s\" using command \"%s\" with param \"%s\"%c",
						ti->handlerArrays[HANDLER_ARRAY_ITEM].buffer,
						ti->handlerArrays[HANDLER_ARRAY_UID].buffer,
						ti->handlerArrays[HANDLER_ARRAY_COMMAND].buffer,
