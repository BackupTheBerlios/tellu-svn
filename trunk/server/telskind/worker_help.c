...
char *listHelp(struct threadInfo * ti) {
	// List all help pages
	return(fetchHelp(1, QUERY_TYPE_PULL, ti));
}

char *pullHelp(struct threadInfo * ti) {
	...
	return(fetchHelp(2, QUERY_TYPE_PULL, ti));
}
...
char *searchHelp(struct threadInfo * ti) {
	...
	return(fetchHelp(3, QUERY_TYPE_PULL, ti));
}
