...
char *listTCP(struct threadInfo * ti) {
	// List all ports
	return(fetchTCP(1, QUERY_TYPE_PULL, ti));
}

char *listUDP(struct threadInfo * ti) {
	// List all ports
	return(fetchUDP(1, QUERY_TYPE_PULL, ti));
}

char *pullTCP(struct threadInfo * ti) {
	...
	// Get requested port
	return(fetchTCP(2, QUERY_TYPE_PULL, ti));
}

char *pullUDP(struct threadInfo * ti) {
	if(ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer == NULL ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer[0] == 0 ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].size == 0) {
		replyPrepare(ERROR_SLIGHT, ERROR_CLASS_GENERAL, ERROR_CODE_GENERAL_PARAMETERNEEDED, ERROR_MESS_GENERAL_PARAMETERNEEDED, ti);
	...
	// Get requested port
	return(fetchUDP(2, QUERY_TYPE_PULL, ti));
}

char *searchTCP(struct threadInfo * ti) {
	if(ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer == NULL ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer[0] == 0 ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].size == 0 ||
	...
	// Search from ports
	return(fetchTCP(3, QUERY_TYPE_PULL, ti));
}

char *searchUDP(struct threadInfo * ti) {
	if(ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer == NULL ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].buffer[0] == 0 ||
	ti->handlerArrays[HANDLER_ARRAY_PARAM].size == 0 ||
	ti->handlerArrays[HANDLER_ARRAY_ITEM].buffer == NULL ||
	...
	// Search from ports
	return(fetchUDP(3, QUERY_TYPE_PULL, ti));
}

char *fetchTCP(int getThis, int getType, struct threadInfo * ti) {
		...
		switch(getThis) {
			case 1:
				...
				snprintf(
					...
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_PORT_TCP " FROM " TABLE_PORT_TCP " ORDER BY " TABLEORD_PORT_TCP_PORT "%c",
					0
				);
			...
			case 2:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_PORT_TCP " FROM " TABLE_PORT_TCP " WHERE " TABLECOL_PORT_TCP_PORT " = '%s'%c",
			...
			case 3:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_PORT_TCP " FROM " TABLE_PORT_TCP " WHERE %s LIKE '%%%s%%' ORDER BY " TABLEORD_PORT_TCP_PORT "%c",
					ti->commandInfo.esc4Buffer,
...
char *fetchUDP(int getThis, int getType, struct threadInfo * ti) {
	int (*dbConnect)(struct threadStorageInfo *);
	void (*dbDisconnect)(struct threadStorageInfo *);
	void (*dbEscape)(struct threadStorageInfo *, char *, char *);
	void (*dbPush)(struct threadStorageInfo *, char *);
		...
		switch(getThis) {
			case 1:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_PORT_UDP " FROM " TABLE_PORT_UDP " ORDER BY " TABLEORD_PORT_UDP_PORT "%c",
					0
				);
			...
			case 2:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_PORT_UDP " FROM " TABLE_PORT_UDP " WHERE " TABLECOL_PORT_UDP_PORT " = '%s'%c",
					ti->commandInfo.esc4Buffer,
					0
				);
			...
			case 3:
				...
				snprintf(
					ti->commandInfo.statBuffer,
					ti->commandInfo.s,
					"SELECT DISTINCT " TABLEKEY_PORT_UDP " FROM " TABLE_PORT_UDP " WHERE %s LIKE '%%%s%%' ORDER BY " TABLEORD_PORT_UDP_PORT "%c",
					ti->commandInfo.esc4Buffer,
					ti->commandInfo.esc2Buffer,
					0
				);
